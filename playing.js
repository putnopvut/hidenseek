var Playing = function(game) {
	...
	this.onDtmfReceived = function(event, participant) {
		...
		if (event.digit == '2') {
			...
			game.notify_observers(JSON.stringify({ type: 'catch_attempt', room: participant.room.id, channel: participant.channel.id, id: participant.id }));
			...
			participant.room.occupants.forEach(function(item) {
				...
				game.seekers++;
				item.role = 'seeker';
				game.notify_observers(JSON.stringify({ type: 'hider_caught', room: participant.room.id, channel: item.channel.id, id: item.id }));
			});
		...
		if (nextRoom == null) {
			...
			game.notify_observers(JSON.stringify({ type: 'invalid_room_move', room: participant.room.id, channel: participant.channel.id, id: participant.id, direction: event.digit }));
			return;
		}
